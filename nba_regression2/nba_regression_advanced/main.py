...
def createTodaysGames(games, df, odds):
    ...
    for game in games:
        ...
        stats['Days-Rest-Home'] = 2#home_days_off.days-1
        stats['Days-Rest-Away'] = 2#away_days_off.days-1
        match_data.append(stats)

    games_data_frame = pd.concat(match_data, ignore_index=True, axis=1)
    games_data_frame = games_data_frame.T

    frame_ml = games_data_frame.drop(columns=['TM_TOV_PCT_RANK','EFG_PCT_RANK','TS_PCT_RANK' ,'PACE_RANK','PIE_RANK',
...
def main():
    odds = {}
    odds = SbrOddsProvider(sportsbook='caesars').get_odds()
