...
con.close()
OU = data['Score']
data.drop(['Score', 'Home-Team-Win', 'TEAM_NAME', 'Date', 'TEAM_NAME.1', 'Date.1', 'OU-Cover','TEAM_ID', 'TEAM_ID.1'],
          axis=1, inplace=True)
...
for x in range(100):
    ...
    for y in range(100):
        ...
        #print("Score:",acc)
        if(acc < max_value):
            print("Save Model:XGBoost_{}_UO_test_2012s.json".format(acc))
            max_value = acc
            model.save_model('../../Models/XGBoost_{}_UO_test_2012s.json'.format(acc))
